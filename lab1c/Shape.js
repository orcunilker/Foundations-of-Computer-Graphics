...
//Added Rotation, Translation and Scaling also for global tranformations
    ...
    /*
    rotateAroundAxis(angle, axes){ 

        let prevTrans = vec3.create();
        let prevTransInverse = vec3.create();
        mat4.getTranslation(prevTrans, this.transformationMatrix);
        vec3.inverse(prevTransInverse, prevTrans)

        this.translateGlobal(prevTransInverse);
        this.rotateGlobal(angle, axes);
        this.translateGlobal(prevTrans);
    }
    */  
